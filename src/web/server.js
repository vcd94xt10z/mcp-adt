    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (url.pathname === "/api/environment-options" && req.method === "GET") {
                ...
                const [packageOptions, targetOptions, requestOptions] = await Promise.all([
    ...
    switch (operation) {
        ...
        case "package_create": {
            const packageInput = {
                ...
                language: normalizeLanguage(config.language), packageType: input.packageType,
        ...
        case "class_create": {
            result = await new ClassApi(client).create({ name: required(input, "name"), description: String(input.description ?? ""), packageName: required(input, "packageName"), transport: optional(input, "transport"), language: normalizeLanguage(config.language), responsible: config.user, final: input.final === undefined ? true : Boolean(input.final), visibility: optional(input, "visibility") ?? "public", source: String(input.source ?? "") });
            break;
        }
        case "class_update":
        ...
        case "interface_create": {
            result = await new InterfaceApi(client).create({ name: required(input, "name"), description: String(input.description ?? ""), packageName: required(input, "packageName"), transport: optional(input, "transport"), language: normalizeLanguage(config.language), responsible: config.user, final: input.final === undefined ? true : Boolean(input.final), visibility: optional(input, "visibility") ?? "public", source: String(input.source ?? "") });
            break;
        }
        ...
        case "domain_create":
            result = await new DomainApi(client).create({ ...input, name: required(input, "name"), packageName: required(input, "packageName"), description: String(input.description ?? ""), language: normalizeLanguage(config.language), responsible: config.user });
            break;
        case "domain_update":
            result = await new DomainApi(client).update({ ...input, name: required(input, "name"), language: normalizeLanguage(config.language), responsible: config.user });
            break;
        ...
        case "report_create":
            result = await new ReportApi(client).create({ name: required(input, "name"), description: String(input.description ?? ""), packageName: required(input, "packageName"), transport: optional(input, "transport"), language: normalizeLanguage(config.language), responsible: config.user, source: String(input.source ?? "") });
            break;
        case "report_update":
            result = await new ReportApi(client).update({ name: required(input, "name"), description: String(input.description ?? ""), packageName: optional(input, "packageName"), transport: optional(input, "transport"), language: normalizeLanguage(config.language), responsible: optional(input, "responsible"), source: String(input.source ?? "") });
            break;
