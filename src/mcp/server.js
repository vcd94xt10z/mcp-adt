...
export function createMcpServer(initialConnections) {
    ...
    server.registerTool("dataelement_list", { title:"List DDIC data elements", description:"Search DDIC data elements.", inputSchema:connectionSchema.extend({query:z.string().optional(),maxResults:z.number().optional()}) }, async ({connection,query,maxResults})=>result(await new DataElementApi(await client(connection)).list({query,maxResults})));
    server.registerTool("dataelement_get", { title:"Get DDIC data element", description:"Read a DDIC data element.", inputSchema:connectionSchema.extend({name:z.string().min(1),version:z.string().optional()}) }, async ({connection,name,version})=>{const api=new DataElementApi(await client(connection));const d=await api.get(name,version);return result({...d,transport:await api.getTransport(d.name,d.packageName).catch(()=>({number:""}))});});
    server.registerTool("dataelement_create", { title:"Create DDIC data element", description:"Create a DDIC data element. Local packages do not require a transport request.", inputSchema:connectionSchema.extend({name:z.string().min(1),description:z.string(),packageName:z.string().min(1),transport:z.string().optional(),typeKind:z.string().optional(),typeName:z.string().optional(),dataType:z.string().optional(),dataTypeLength:z.number().optional(),dataTypeDecimals:z.number().optional(),shortFieldLabel:z.string().optional(),mediumFieldLabel:z.string().optional(),longFieldLabel:z.string().optional(),headingFieldLabel:z.string().optional()}) }, async ({connection,...input})=>{const config=await connectionConfig(connection);return result(await new DataElementApi(await client(connection)).create({...input,language:connectionLanguage(config),responsible:config.user}));});
    server.registerTool("dataelement_update", { title:"Update DDIC data element", description:"Update a DDIC data element.", inputSchema:connectionSchema.extend({name:z.string().min(1),description:z.string().optional(),packageName:z.string().optional(),transport:z.string().optional(),typeKind:z.string().optional(),typeName:z.string().optional(),dataType:z.string().optional(),dataTypeLength:z.number().optional(),dataTypeDecimals:z.number().optional(),shortFieldLabel:z.string().optional(),mediumFieldLabel:z.string().optional(),longFieldLabel:z.string().optional(),headingFieldLabel:z.string().optional()}) }, async ({connection,...input})=>{const config=await connectionConfig(connection);return result(await new DataElementApi(await client(connection)).update({...input,language:connectionLanguage(config),responsible:config.user}));});
    server.registerTool("dataelement_activate", { title:"Activate DDIC data element", description:"Activate a DDIC data element.", inputSchema:connectionSchema.extend({name:z.string().min(1)}) }, async ({connection,name})=>{const n=name.trim().toUpperCase();return result(await new ActivationApi(await client(connection)).activate({uri:`/sap/bc/adt/ddic/dataelements/${encodeURIComponent(n.toLowerCase())}`,name:n}));});
    server.registerTool("dataelement_delete", { title:"Delete DDIC data element", description:"Delete a DDIC data element.", inputSchema:connectionSchema.extend({name:z.string().min(1),transport:z.string().optional()}) }, async ({connection,name,transport})=>result(await new DataElementApi(await client(connection)).delete(name,transport)));
