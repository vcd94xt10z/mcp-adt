...
export class PackageApi {
    ...
    async list({ query = "Z*", superPackage = "", maxResults = 100 } = {}) {
        ...
        const items = parsePackageSearch(response.body).filter(item => !searchQuery || searchQuery === "*" || wildcardMatch(item.name, searchQuery)).filter(item => !parentFilter || item.superPackage.toUpperCase() === parentFilter.toUpperCase());
        return { status: response.status, durationMs: response.durationMs, query: searchQuery, superPackage: parentFilter, maxResults: limit, count: items.length, items, raw: response.body };
    }
